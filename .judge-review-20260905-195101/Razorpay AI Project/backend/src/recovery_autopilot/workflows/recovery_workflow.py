...
class RecoveryWorkflow:
    ...
    async def execute_approved_action(self, case: PaymentCase) -> ExecutionResult:
        ...
        if settings.KILL_SWITCH_ACTIVE:
            logger.warning("Emergency Kill Switch is ACTIVE. Blocking execution for case %s.", case.case_id)
            if case.status != CaseStatus.AWAITING_APPROVAL:
                audit_hold = CaseStateMachine.transition(
                    case, CaseStatus.AWAITING_APPROVAL, actor=ActorType.POLICY, reason="Emergency Kill Switch ACTIVE"
                )
                await self._record_audit(audit_hold)
                await self._save_case(case)
    ...
    async def handle_payment_success(
        ...
        amount_inr: float,
        currency: str = "INR",
        matched_field: str = "payment_id",
        matched_value: str = "",
    ) -> PaymentOutcome:
        """Handle incoming payment.captured or order.paid: validate financial correctness and idempotency."""
        # 1. Idempotency Guard: if already recovered with same outcome, do not double-count
        if case.status == CaseStatus.RECOVERED and case.outcome and case.outcome.recovered:
            logger.info("Case %s is already marked RECOVERED. Duplicate capture event ignored.", case.case_id)
            ...
            await self._record_audit(audit_dup)
            return case.outcome

        # 2. Financial Amount & Currency Validation
        currency_match = currency.upper() == case.context.currency.upper()
        amount_diff = abs(amount_inr - case.context.amount_inr)
        amount_valid = amount_diff <= 0.05  # Within 5 paise tolerance

        if not currency_match or not amount_valid:
            logger.warning(
                "Financial discrepancy detected on Case %s: Expected %s %s, received %s %s. Escalating to human review.",
                case.case_id,
            ...
            audit_mismatch = AuditEvent(
                case_id=case.case_id,
                actor=ActorType.POLICY,
                event_type="PAYMENT_AMOUNT_MISMATCH_DETECTED",
                details={
                    "expected_amount": case.context.amount_inr,
                    "received_amount": amount_inr,
                    "expected_currency": case.context.currency,
        ...
        # 3. Successful Validated Recovery
        ...
        audit_trans = CaseStateMachine.transition(
            ...
            actor=ActorType.POLICY,
            reason=f"Payment captured and verified via exact {matched_field} match",
        )
    ...
    # Method alias for worker compatibility
    initiate_case = process_failed_payment
