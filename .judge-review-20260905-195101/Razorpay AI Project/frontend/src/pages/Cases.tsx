...
export default function Cases() {
  ...
  const load = async () => {
    try {
      ...
      setCases(Array.isArray(data) ? data : ((data as any)?.cases || []));
    } catch (e: any) {
  ...
  const filtered = (Array.isArray(cases) ? cases : []).filter((c) => {
    ...
    const customerId = (c.context?.customer_id || '').toLowerCase();
    const customerEmail = (c.context?.customer_email || '').toLowerCase();
    const customerName = (c.context?.customer_name || '').toLowerCase();
    return (
      caseId.includes(q) ||
  ...
  const columns: ColumnsType<PaymentCase> = [
      ...
      render: (_, record) => (
        <div>
          <div className="font-semibold text-slate-800 text-xs">
            {record.context?.customer_name || record.context?.customer_id || 'Unknown Customer'}
          </div>
    ...
    },
    {
      ...
      key: 'amount',
      sorter: (a, b) => (a.context?.amount_inr || 0) - (b.context?.amount_inr || 0),
      render: (amt: number) => (
