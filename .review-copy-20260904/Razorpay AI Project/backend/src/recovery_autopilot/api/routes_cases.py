...
async def approve_case(case_id: str, req: ApproveRequest, db: AsyncSession = Depends(get_db)):
    """Approve a case held for human operator review."""
    repo = SqlAlchemyRepository(db)
    case = await repo.get_case(case_id)
    if not case:
        raise HTTPException(status_code=404, detail="Case not found")
    ...
    try:
        await workflow.handle_human_approval(case, operator_id=req.operator_id)
        await db.commit()
...
async def reject_case(case_id: str, req: RejectRequest, db: AsyncSession = Depends(get_db)):
    """Reject a case held for human operator review and stop recovery."""
    repo = SqlAlchemyRepository(db)
    case = await repo.get_case(case_id)
    if not case:
        raise HTTPException(status_code=404, detail="Case not found")
    ...
    workflow = orchestrator.create_workflow(repo)
    try:
        await workflow.handle_human_rejection(case, operator_id=req.operator_id, reason=req.reason)
        await db.commit()
