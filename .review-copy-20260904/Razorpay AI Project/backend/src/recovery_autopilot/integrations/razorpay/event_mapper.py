...
class RazorpayEventMapper:
    ...
    def map_payment_failed(payload: Dict[str, Any]) -> PaymentContext:
        ...
        return PaymentContext(
            ...
            subscription_id=payment_entity.get("subscription_id") or payload.get("payload", {}).get("subscription", {}).get("entity", {}).get("id") or "sub_unknown",
            invoice_id=payment_entity.get("invoice_id"),
            customer_id=payment_entity.get("customer_id") or "cust_syn_guest",
            customer_name=payment_entity.get("notes", {}).get("customer_name") or "Synthetic Customer",
            customer_email=payment_entity.get("email") or "customer@synthetic-test.example.com",
            customer_phone=payment_entity.get("contact") or "+919800000000",
    ...
    @staticmethod
    def map_payment_captured(payload: Dict[str, Any]) -> Tuple[str, float]:
        """Extract (payment_id, amount_inr) from payment.captured / order.paid webhook."""
        payment_entity = payload.get("payload", {}).get("payment", {}).get("entity", {})
        pay_id = payment_entity.get("id", "pay_captured")
        amount_paise = payment_entity.get("amount", 0)
        amount_inr = round(float(amount_paise) / 100.0, 2)
        return pay_id, amount_inr
